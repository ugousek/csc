...
<main id="main" class="xevos-main xevos-main--glows">
	...
	<!-- 1. Hero -->
	<?php
	$_show = get_field('on_hero_zobrazit');
	if ($_show !== '0' && $_show !== 0) :
		$_bg = get_field('on_hero_background');
		get_template_part('template-parts/components/hero-page', null, [
			'heading'     => get_field('on_hero_heading') ?: 'Kybernetické testování',
			'description' => get_field('on_hero_subheading') ?: 'Kybernetické testování je soubor praktických technik, které simulují reálné útoky s cílem odhalit zranitelnosti dříve, než je objeví skutečný útočník a to s využitím nejmodernějšího softwaru, pokročilé automatizace a umělé inteligence.',
	...
	<!-- 2. Kyber test slider -->
	<?php if (get_field('on_slider_zobrazit') !== false) : ?>
		<?php get_template_part('template-parts/components/kyber-testovani'); ?>
	<?php endif; ?>

		<!-- 3. Postup testování -->
		<?php
		$postup_heading = get_field('on_postup_heading') ?: 'Jak testování probíhá?';
		$postup_text    = get_field('on_postup_text') ?: 'Testování probíhá v jasně definovaných krocích – od domluvy rozsahu až po finální zprávu a ověření oprav. Díky tomu <strong>máte průběžný přehled</strong> o tom, co se děje a jaké budou výstupy.';
		$kroky = get_field('on_postup_kroky');
		if (! $kroky) {
			$kroky = [
		...
		?>
		<?php if (get_field('on_postup_zobrazit') !== false) : ?>
		...
		<!-- 4. Co získáte / Pro koho -->
		<?php
		$ziskate_heading = get_field('on_ziskate_heading') ?: 'Co získáte?';
		$ziskate_text    = get_field('on_ziskate_text') ?: 'Kybernetická politika vytváří <strong>jasný rámec pro řízení bezpečnosti v organizaci.</strong> Určuje <strong>role, odpovědnosti a pravidla,</strong> podle kterých se chrání data, systémy a provoz.';
		$ziskate_seznam  = get_field('on_ziskate_seznam');
		$prokoho_heading = get_field('on_prokoho_heading') ?: 'Pro koho je služba vhodná?';
		$prokoho_text    = get_field('on_prokoho_text') ?: 'Kybernetická politika vytváří <strong>jasný rámec pro řízení bezpečnosti v organizaci.</strong> Určuje <strong>role, odpovědnosti a pravidla,</strong> podle kterých se chrání data, systémy a provoz.';
		$prokoho_seznam  = get_field('on_prokoho_seznam');
		$benefity_cta_text = get_field('on_benefity_cta_text') ?: 'CHCI TEST';
		$benefity_cta_url  = get_field('on_benefity_cta_url') ?: '#formular';
		...
		?>
		<?php if (get_field('on_benefity_zobrazit') !== false) : ?>
	...
	<!-- 5. Banner -->
	<?php if (get_field('on_banner_zobrazit') !== false) : ?>
		<?php
		$banner_heading  = get_field('on_banner_heading') ?: 'Chcete ověřit, jak je vaše organizace odolná?';
		$banner_text     = get_field('on_banner_text') ?: 'Provedeme testování s využitím <strong>nejmodernějších nástrojů,</strong> aby výsledky odpovídaly aktuálním hrozbám i metodikám.';
		$banner_btn_text = get_field('on_banner_btn_text') ?: 'POTŘEBUJI POMOC!';
	...
	<!-- 6. Aktuality -->
	<?php if (get_field('on_aktuality_zobrazit') !== false) : ?>
		<?php get_template_part('template-parts/components/aktuality'); ?>
	<?php endif; ?>

	<!-- 7. Recenze -->
	<?php if (get_field('on_recenze_zobrazit') !== false) : ?>
		<?php get_template_part('template-parts/components/recenze'); ?>
	<?php endif; ?>

	<!-- 8. Formulář -->
	<?php if (get_field('on_formular_zobrazit') !== false) : ?>
		<?php
		$form_heading = get_field('on_formular_heading') ?: 'Poptávka testování';
		$form_text    = get_field('on_formular_text') ?: 'Spolupracujeme s firmami, které chtějí mít v <strong>kybernetické bezpečnosti jasno</strong>.';
		?>
