...
<main id="main" class="xevos-main">
	...
	<!-- 3. Postup testování -->
	<?php
	$postup_heading = get_field('kt_postup_heading') ?: 'Jak testování probíhá?';
	$postup_text    = get_field('kt_postup_text') ?: 'Testování probíhá v jasně definovaných krocích – od domluvy rozsahu až po finální zprávu a ověření oprav. Díky tomu <strong>máte průběžný přehled</strong> o tom, co se děje a jaké budou výstupy.';
	...
	<section class="xevos-section xevos-postup-section">
		<div class="xevos-section__container">
			...
			<div class="xevos-postup-grid">
				<?php foreach ($kroky as $i => $k) : ?>
					<div class="xevos-postup-step">
						<span class="xevos-postup-step__num"><?php echo esc_html($i + 1); ?>.</span>
						<h4 class="xevos-postup-step__title"><?php echo esc_html($k['nazev'] ?? ''); ?></h4>
						<p class="xevos-postup-step__text"><?php echo esc_html($k['popis'] ?? ''); ?></p>
					</div>
				<?php endforeach; ?>
			</div>
		</div>
	...
	<!-- 4. Co získáte / Pro koho -->
	<?php
	$ziskate_heading = get_field('kt_ziskate_heading') ?: 'Co získáte?';
	$ziskate_text    = get_field('kt_ziskate_text') ?: 'Kybernetická politika vytváří <strong>jasný rámec pro řízení bezpečnosti v organizaci.</strong> Určuje <strong>role, odpovědnosti a pravidla,</strong> podle kterých se chrání data, systémy a provoz.';
	$ziskate_seznam  = get_field('kt_ziskate_seznam');
	$prokoho_heading = get_field('kt_prokoho_heading') ?: 'Pro koho je služba vhodná?';
	$prokoho_text    = get_field('kt_prokoho_text') ?: 'Kybernetická politika vytváří <strong>jasný rámec pro řízení bezpečnosti v organizaci.</strong> Určuje <strong>role, odpovědnosti a pravidla,</strong> podle kterých se chrání data, systémy a provoz.';
	$prokoho_seznam  = get_field('kt_prokoho_seznam');
	$benefity_cta_text = get_field('kt_benefity_cta_text') ?: 'CHCI TEST';
	...
	<!-- 5. Banner -->
	<?php
	$banner_heading  = get_field('kt_banner_heading') ?: 'Chcete ověřit, jak je vaše organizace odolná?';
	$banner_text     = get_field('kt_banner_text') ?: 'Provedeme testování s využitím <strong>nejmodernějších nástrojů,</strong> aby výsledky odpovídaly aktuálním hrozbám i metodikám.';
	$banner_btn_text = get_field('kt_banner_btn_text') ?: 'POTŘEBUJI POMOC!';
	...
	<!-- 8. Formulář -->
	<?php
	$form_heading = get_field('kt_formular_heading') ?: 'Poptávka testování';
	$form_text    = get_field('kt_formular_text') ?: 'Spolupracujeme s firmami, které chtějí mít v <strong>kybernetické bezpečnosti jasno</strong>.';
	?>
	<section class="xevos-section" id="formular">
		<div class="xevos-section__container">
			...
			<div class="xevos-order-section">
				<div class="xevos-kontakt__form">
					<?php $cf7 = get_field('kt_formular_shortcode');
					if ($cf7) : echo do_shortcode($cf7);
					else : ?>
						<form method="post">
							<div class="xevos-form-row">
								<div class="xevos-form__group">
									...
									<input type="text" class="xevos-form__input" required>
								</div>
								<div class="xevos-form__group">
							...
							<div class="xevos-form-row">
								<div class="xevos-form__group">
									...
									<input type="tel" class="xevos-form__input" required>
								</div>
								<div class="xevos-form__group">
									<label class="xevos-form__label">E-mail <span class="xevos-form__required">*</span></label>
									<input type="email" class="xevos-form__input" required>
								</div>
							</div>
							<div class="xevos-form-row">
								<div class="xevos-form__group">
									<label class="xevos-form__label">Druh testu <span class="xevos-form__required">*</span></label>
									<select class="xevos-form__input">
										<option>Kybernetická bezpečnost – obecná</option>
										<option>Penetrační testy</option>
										<option>Audit infrastruktury</option>
									</select>
								</div>
							</div>
							<div class="xevos-form__group">
								<label class="xevos-form__label">Zpráva</label>
								<textarea class="xevos-form__textarea" rows="4"></textarea>
							</div>
