...
function xevos_cookie_consent_banner(): void {
	...
	<div class="xevos-cookie-banner" id="cookie-banner" style="display:none;">
		<div class="xevos-cookie-banner__text">
			<svg width="20" height="20" viewBox="0 0 20 20" fill="none" style="flex-shrink:0;">
				<circle cx="10" cy="10" r="9" stroke="var(--color-text-dim)" stroke-width="1.5"/>
				<circle cx="7" cy="8" r="1.5" fill="var(--color-text-dim)"/>
				<circle cx="13" cy="7" r="1" fill="var(--color-text-dim)"/>
				<circle cx="10" cy="13" r="1.5" fill="var(--color-text-dim)"/>
				<circle cx="14" cy="12" r="1" fill="var(--color-text-dim)"/>
			</svg>
	...
	(function() {
		...
		document.getElementById('cookie-accept').addEventListener('click', function() { setCookieConsent('accepted'); });
		document.getElementById('cookie-reject').addEventListener('click', function() { setCookieConsent('rejected'); });
	})();
	</script>
	<?php
}
