...
<main id="main" class="xevos-main xevos-main--glows">
	...
	<!-- 1. Hero -->
	<?php
	$_show = get_field('nis_hero_zobrazit');
	if ($_show !== '0' && $_show !== 0) :
		$_bg = get_field('nis_hero_background');
	...
	<!-- 2. Kyber test slider -->
	<?php if (get_field('nis_slider_zobrazit') !== false) : ?>
		<?php get_template_part('template-parts/components/kyber-testovani'); ?>
	<?php endif; ?>

		<!-- 3. Postup testování -->
		<?php
		$postup_heading = get_field('nis_postup_heading') ?: 'Jak testování probíhá?';
		$postup_text    = get_field('nis_postup_text') ?: 'Testování probíhá v jasně definovaných krocích – od domluvy rozsahu až po finální zprávu a ověření oprav. Díky tomu <strong>máte průběžný přehled</strong> o tom, co se děje a jaké budou výstupy.';
		$kroky = get_field('nis_postup_kroky');
		if (! $kroky) {
			$kroky = [
		...
		?>
		<?php if (get_field('nis_postup_zobrazit') !== false) : ?>
		...
		<!-- 4. Co získáte / Pro koho -->
		<?php
		$ziskate_heading = get_field('nis_ziskate_heading') ?: 'Co získáte?';
		$ziskate_text    = get_field('nis_ziskate_text') ?: 'Kybernetická politika vytváří <strong>jasný rámec pro řízení bezpečnosti v organizaci.</strong> Určuje <strong>role, odpovědnosti a pravidla,</strong> podle kterých se chrání data, systémy a provoz.';
		$ziskate_seznam  = get_field('nis_ziskate_seznam');
		$prokoho_heading = get_field('nis_prokoho_heading') ?: 'Pro koho je služba vhodná?';
		$prokoho_text    = get_field('nis_prokoho_text') ?: 'Kybernetická politika vytváří <strong>jasný rámec pro řízení bezpečnosti v organizaci.</strong> Určuje <strong>role, odpovědnosti a pravidla,</strong> podle kterých se chrání data, systémy a provoz.';
		$prokoho_seznam  = get_field('nis_prokoho_seznam');
		$benefity_cta_text = get_field('nis_benefity_cta_text') ?: 'CHCI TEST';
		$benefity_cta_url  = get_field('nis_benefity_cta_url') ?: '#formular';
		...
		?>
		<?php if (get_field('nis_benefity_zobrazit') !== false) : ?>
	...
	<!-- 5. Banner -->
	<?php if (get_field('nis_banner_zobrazit') !== false) : ?>
		<?php
		$banner_heading  = get_field('nis_banner_heading') ?: 'Chcete ověřit, jak je vaše organizace odolná?';
		$banner_text     = get_field('nis_banner_text') ?: 'Provedeme testování s využitím <strong>nejmodernějších nástrojů,</strong> aby výsledky odpovídaly aktuálním hrozbám i metodikám.';
		$banner_btn_text = get_field('nis_banner_btn_text') ?: 'POTŘEBUJI POMOC!';
		$banner_btn_url  = get_field('nis_banner_btn_url') ?: '#formular';
		$banner_image    = get_field('nis_banner_image');
	...
	<!-- 6. Aktuality -->
	<?php if (get_field('nis_aktuality_zobrazit') !== false) : ?>
		<?php get_template_part('template-parts/components/aktuality'); ?>
	<?php endif; ?>

	<!-- 7. Recenze -->
	<?php if (get_field('nis_recenze_zobrazit') !== false) : ?>
		<?php get_template_part('template-parts/components/recenze'); ?>
	<?php endif; ?>

	<!-- 8. Formulář -->
	<?php if (get_field('nis_formular_zobrazit') !== false) : ?>
		<?php
		$form_heading = get_field('nis_formular_heading') ?: 'Poptávka testování';
		$form_text    = get_field('nis_formular_text') ?: 'Spolupracujeme s firmami, které chtějí mít v <strong>kybernetické bezpečnosti jasno</strong>.';
		?>
